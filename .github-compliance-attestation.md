...
| **Pinned, reviewed actions** | ✅ | Third-party actions are pinned to reviewed major versions (e.g. `actions/checkout@v7`) and limited to first-party GitHub actions where practical. |
...
| **Resource limits / DoS protection** | ✅ | The workflow has a 30-minute job timeout, and `limits.workflowTimeoutMinutes` independently bounds the OpenClaw subprocess (≤ 360). |
...
| No automated excessive resource use | ✅ | Hard caps at both the workflow-job and OpenClaw-subprocess levels. Workflow runs are bounded and ephemeral. |
